...
const addToCart = async function (req, res) {
    try {
        ...
        if (!validator.isValidRequestValue(req.body.items)) return res.status(400).send({ status: false, message: ` Key Name : 'items' You can pass only a valid iteam. Make sure you can not pass only key name or a blank key` })
        ...
        if (!validator.isValidRequestValue(productId)) return res.status(400).send({ status: false, message: ` Key Name : 'productId' You can pass only a valid ObjectId. Make sure you can not pass only key name or a blank key` })
        ...
        if (!validator.isValidRequestValue(quantity)) return res.status(400).send({ status: false, message: ` Key Name : 'quantity' You can pass only a valid quantity. Make sure you can not pass only key name or a blank key` })
...
const removeProductFromCart = async function (req, res) {
    try {
        ...
        if (!validator.isValidRequestValue(removeProduct)) return res.status(400).send({ status: false, message: ` Key Name : 'removeProduct' You can pass only a valid key. Make sure you can not pass only key name or a blank key` })
